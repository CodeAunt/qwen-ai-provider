...
import { QwenCompletionLanguageModel } from "../src/qwen-completion-language-model"
import { QwenEmbeddingModel } from "../src/qwen-embedding-model"
import { createQwen } from "../src/qwen-provider"

vi.stubEnv("DASHSCOPE_API_KEY", "test-api-key-123")

// Mock the model classes
vi.mock("../src/qwen-chat-language-model", () => ({
  QwenChatLanguageModel: vi.fn(),
}))

vi.mock("../src/qwen-completion-language-model", () => ({
  QwenCompletionLanguageModel: vi.fn(),
}))

vi.mock("../src/qwen-embedding-model", () => ({
